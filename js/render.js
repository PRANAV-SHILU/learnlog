...
function accordion({ id, faIcon, title, subtitle, count, content, countLabel }) {
...
export function renderSkills(targetEl) {
  ...
  const catHtml = skillCategories.map(cat => {
...
export function renderTools(targetEl) {
  ...
  const categoriesHtml = toolCategories.map(cat => {
...
export function renderPlatforms(targetEl) {
  ...
  const items = platforms.map(p => {
    ...
    const topicsHtml = p.topics.map(t => `<span class="topic-tag">${t}</span>`).join('');
...
export function renderProjects(targetEl) {
  const items = projects.map(proj => {
    const stackHtml = proj.stack.map(t => `<span class="stack-tag">${t}</span>`).join('');
...
export function renderRoadmap(targetEl) {
  ...
  const labels   = { done: '✅ Completed', learning: '🔄 In Progress', planned: '🗓️ Planned' };

  const columnsHtml = statuses.map(status => {
    const items = roadmap.filter(r => r.status === status).sort((a, b) => a.order - b.order);
...
export function renderHeroSocial() {
  ...
  if (!el) return;
  el.innerHTML = about.social.map(s => `
